...
class Diffusion:
    ...
    def output(self, prec_no, block_no, start_date, end_date, csvdir=None):
        ...
        df.to_csv('C:/Users/maruko/OneDrive - 愛媛大学 (1)/02_PCSE/tomgrosim-on-pcse/Diffusion.csv')
        return(df)
        

ins = Diffusion()
df = ins.output("51",  "47636",datetime.date(2019,12,25) , datetime.date(2020,6,5))

df_2hour = pd.DataFrame(columns=["date","hour","h","FRACDF"])
def get_2hour_ave(df_2hour,df):
    rows = len(df)
    for ii,i in enumerate(range(1,rows,2)):
        date = df.loc[i,["date"]] 
        date = date[0]
        hour = df.loc[i,["hour"]]
        hour = hour[0]
        ...
        h2 = h2[0]        
        h = (h1+h2)/2
        FRACDF1 = df.loc[i-1,["FRACDF"]]
        FRACDF1 = FRACDF1[0]
        FRACDF2 = df.loc[i,["FRACDF"]]
        FRACDF2 = FRACDF2[0]
        FRACDF = (FRACDF1+FRACDF2)/2
        # print(date,hour,h,FRACDF)
        # data = pd.Series({"date":date,"hour":hour,"h":h,"FRACDF":FRACDF})
        # df_2hour.append(data, ignore_index=True)
        df_2hour.loc[ii] = [date,hour,h,FRACDF]
    return df_2hour 
# print(df_2hour)
df_2hour = get_2hour_ave(df_2hour,df)
df_2hour.to_csv('C:/Users/maruko/OneDrive - 愛媛大学 (1)/02_PCSE/tomgrosim-on-pcse/Diffusion_2hour.csv')
