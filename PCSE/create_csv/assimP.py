...
import pandas as pd
#20,400,molの時のEFF＿，PGMAX_，RD20 = 1.136，
def LPHCUR(TLEAF,CO2AIR,unit):
  RS = 50 # [s m-1]気孔抵抗
  RB = 100 # [s m-1]葉面境界層抵抗
  Q10RD = 2.0
  ...
  if unit == "g":
    ...
    RD20 = 1.136 # [µmolCO2 m-2 s-1]#暗呼吸速度
    GCT_X = [0, 5, 15, 25, 40, 100]
  ...
  if unit == "g":
    ...
    ALPHA = 41.6
  PNC = ALPHA * (CO2 - GAMMA) / (1.36 * RB + 1.6 * RS + RC)#そのCO2濃度下と抵抗下での最大値
  PNMAX = min(PNC, PMM)
